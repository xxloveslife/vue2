    ...
    <section class="w mt30 clearfix" v-if='item.type===2'>
      <MShelf :title='item.name'>
        <div slot="content" class='hot' >
          <MallGoods :goods=o v-for='(o,i) in item.panelContents' :key=i></MallGoods>
        </div>
      </MShelf>
    </section>
      ...
      <MShelf :title='item.name'>
        <div slot='content' class="floors">
          ...
          <div class="imgbanner" v-for='(o,j) in item.panelContents' :key=j v-if='o.type===2 || o.type===3'>
            <img :src='o.productImageBig'>
          </div>
          <MallGoods :goods=o v-for='(o,j) in item.panelContents' :key=j v-if='o.type===0'>
  ...
  async created() {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        let item = data.find(item => item.type === 0);
